...
from openstereo.ui.smallcircle_properties_ui import Ui_Dialog as\
    smallcircle_Ui_Dialog
from openstereo.ui.circular_properties_ui import Ui_Dialog as\
    circular_Ui_Dialog
...
class DataItem(QtWidgets.QTreeWidgetItem):
    ...
    def items(self):
        items = [
            item_name[5:] for item_name in dir(self)
    ...
    def checked_plots(self):
        for item_name in self.items:
            if self.check_items[self.plot_item_name.get(
                    item_name, item_name)].checkState(0):
    ...
    def get_checked_status(self):
        return {
            self.plot_item_name.get(item_name, item_name):
            bool(self.check_items[self.plot_item_name.get(
                item_name, item_name)].checkState(0))
            for item_name in self.items
        }
    ...
    def build_items(self):
        for item_name in self.items:
            self.add_item(
                self.plot_item_name.get(item_name, item_name).replace(
                    "_", " "))
    ...
    def add_item(self, item_name):
        ...
        item_widget.setFlags(QtCore.Qt.ItemIsUserCheckable | QtCore.Qt.
                             ItemIsEnabled | QtCore.Qt.ItemIsSelectable)
...
class CircularData(DataItem):
    ...
    def plot_Rose(self):
        ...
        if self.rose_check_settings["standard"]:
            nodes = grid.build_grid(self.rose_settings["binwidth"],
                                    self.rose_settings["offset"], from_, to_)
            ...
            nodes = grid.build_grid(self.rose_settings["spacing"],
                                    self.rose_settings["offsetcont"], from_,
                                    to_)
        ...
        if self.rose_check_settings["mean"]:
            ...
            if self.rose_check_settings["dir"]:
                theta -= 90.
            plot_data.append(
                RoseMeanPlotData(theta, confidence, self.rose_check_settings[
                    "diraxial"], self.rose_mean_settings))
        return plot_data
...
class AttitudeData(CircularData):
    ...
    def build_configuration(self):
        ...
        self.v1point_settings = {
            'marker': '*',
            'c': '#00FF00',
            'ms': 12.0,
        }
        self.v2point_settings = {
            'marker': '*',
            'c': '#FF0000',
            'ms': 12.0,
        }
        self.v3point_settings = {
            'marker': '*',
            'c': '#0000FF',
            'ms': 12.0,
        }
    ...
    def _plot_SC(self):
        ...
        if self.check_settings["scaxis"] or self.check_settings["sccirc"]:
            ...
            if self.check_settings["scaxis"]:
                ...
                plot_items.append(
                    PointPlotData(axis, self.scaxis_settings, self.
                                  checklegend_settings['scaxis'], legend_text))
            if self.check_settings["sccirc"]:
                ...
                plot_items.append(
                    CirclePlotData(
                        small_circle(axis, alpha), self.sccirc_settings,
    ...
    def _plot_Mean(self):
        if self.check_settings['meanpoint']:
            if self.legend_settings['meanpoint']:
                try:
                    legend_text = \
                        self.legend_settings['meanpoint'].format(
                            data=self.auttitude_data)
                except:
            ...
            return (PointPlotData(
                ...
                if not self.check_settings['meanpointeiv'] else
                self.auttitude_data.concentrated_mean_vector,
                self.meanpoint_settings,
                self.checklegend_settings['meanpoint'], legend_text), )
        else:
            return []
        ...
        for i, eiv in enumerate(("v1point", "v2point", "v3point")):
            if self.check_settings[eiv]:
                if self.legend_settings[eiv]:
                    try:
                        legend_text = \
                            self.legend_settings[eiv].format(
                                data=self.auttitude_data)
                    except:
                ...
                plot_data.append(
                    PointPlotData(self.auttitude_data.eigenvectors[i],
                                  self.get_item_props(eiv),
                                  self.checklegend_settings[eiv], legend_text))
            ...
            if self.check_settings[eiv]:
                ...
                plot_data.append(
                    CirclePlotData(
                        ...
                        self.get_item_props(eiv), self.checklegend_settings[
                            eiv], legend_text))
        return plot_data
                ...
                if self.contour_check_settings["robinjowett"]:
                    ...
                    try:
                        ...
                        QtWidgets.QMessageBox.warning(
                            self.parent(), "Scipy not Found",
                            "Scipy is needed for Diggle & Fisher method. Defaulting to Robin & Jowett.")  # noqa: E501
                        k = None
            else:
    ...
    def _plot_Classification(self):
        if self.legend_settings['point']:
            ...
            legend_text = "{} ({})".format(
                self.text(0), self.plot_item_name.get('Points', 'Points'))
        return (ClassificationPlotData(
            self.auttitude_data.vollmer_G, self.auttitude_data.vollmer_R,
            self.auttitude_data.woodcock_Kx, self.auttitude_data.woodcock_Ky,
            self.point_settings, self.checklegend_settings['point'],
            legend_text), )
...
class PlaneData(AttitudeData):
    ...
    def plot_GC(self):
        circles = [
            great_circle(point) for point in self.auttitude_data.data]
