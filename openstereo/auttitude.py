...
rotation_to_direction = np.array((( 0.,   1.),\
                                  (-1.,  0.)))

def dcos(data):
    """Converts poles into direction cossines."""
    theta, phi = np.radians(data.T)
    return np.array((-np.sin(phi)*np.sin(theta), -np.sin(phi)*np.cos(theta), -np.cos(phi))).T
...
def sphere(data):
    ...
    z = np.clip(z, -1., 1.)
    return np.array((np.degrees(np.arctan2(sign_z*x,sign_z*y)) % 360, np.degrees(np.arccos(np.abs(z))))).T
...
def intersect(A, B):
    ...
    all_intersections = np.array([np.cross(a, b) for a, b in itertools.product(A.data, B.data)])
    intersections = all_intersections[np.nonzero(np.linalg.norm(all_intersections, axis=1))]
...
Builds a regular grid over the hemisphere, with the given average node spacing."""
    ...
    for phi in np.linspace(node_spacing, 90., 90./node_spacing, endpoint=False):
        azimuth_spacing = math.degrees(2*math.asin((math.sin(spacing/2)/math.sin(math.radians(phi)))))
...
def universal_translator(data, longitude_column=0, colatitude_column=1,\
    ...
    converted_data = np.array(translator.process_data(attitude_data, dd=dip_direction))
    if circular:
...
interpret data as lines, instead of planes."""
    ...
    if isinstance(fin, str):
        extension = extension if extension is not None else os.path.splitext(fin)[-1]
        input_data = universal_loader(fin, extension=extension, worksheet=worksheet)
    else:
        input_data = fin
...
def rotate(data, u, theta):
    return DirectionalData(np.dot(data.data, rotation_matrix(u, theta)), *data.args, **data.kwargs)

def project(data, new_axes):
    return DirectionalData(np.dot(data.data, new_axes.T), *data.args, **data.kwargs)
...
class DirectionalData(object):
    def __init__(self, data, *args, **kwargs):
        ...
        if self.d == 3:
            ...
            if self.data_circle is None:
                circular_data = self.data[:,:2]/np.linalg.norm(self.data[:,:2], axis=1)[:, None]
                circular_data = circular_data[np.isfinite(circular_data).all(axis=1)]
                if not self.kwargs.get('line'): circular_data = -circular_data
                self.data_circle = circle(circular_data, kwargs.get('axial', False))
        else:
            self.data_sphere = kwargs.get('data_sphere',\
        ...
        if kwargs.get('calculate_statistics', True): self.initialize_statistics()
        self._grid = None
        self._cgrid= None
    ...
    def initialize_statistics(self):
        ...
        self.mean_vector = self.resultant_vector/np.linalg.norm(self.resultant_vector)
        ...
        if self.d == 3:
            ...
            self.fisher_k = (self.n - 1)/(self.n - np.linalg.norm(self.resultant_vector))
            ...
            self.concentrated_mean_vector = np.mean(\
                ...
                axis=0)
            self.concentrated_mean_vector /= np.linalg.norm(self.concentrated_mean_vector)

            #From Vollmer 1990
            self.vollmer_P = (self.eigenvalues[0] - self.eigenvalues[1])/eigenvalues.sum()
            self.vollmer_G = 2*(self.eigenvalues[1] - self.eigenvalues[2])/eigenvalues.sum()
            ...
            self.vollmer_C = math.log(self.eigenvalues[0]/self.eigenvalues[2])
            
            #From Woodcock 1977
            self.woodcock_Kx = math.log(self.eigenvalues[1]/self.eigenvalues[2])
            self.woodcock_Ky = math.log(self.eigenvalues[0]/self.eigenvalues[1])
            self.woodcock_C = math.log(self.eigenvalues[0]/self.eigenvalues[2])
            ...
            self.circular_resultant_length = np.linalg.norm(self.circular_resultant_vector)
        ...
        self.circular_standard_deviation = math.sqrt(-2*math.log(1 - self.circular_variance))
    ...
    def estimate_circular_confidence(self, axial=False, alpha=.95):
        if axial:
            ...
            dcos = np.array((np.cos(np.radians(theta)), np.sin(np.radians(theta)))).T
    ...
    def __repr__(self):
        return "%s(%s, *%s, **%s)" %  (self.__class__, self.data, self.args, self.kwargs)
    def __str__(self):
        if self.d == 2:
...
""".format(self=self, filename=self.kwargs.get('filename', ''),
          ...
          resultant=circle(self.resultant_vector, self.kwargs.get('axial', False)))
        elif self.d == 3:
            statistics = """\
{filename}\
...
""".format(*self.mode_sphere)
    ...
    @property
    def source(self):
        return "\n".join("{}: {}".format(arg, self.kwargs[arg]) for arg in self.kwargs)
...
splitting it's first dimension among the available cores."""
    ...
    def parallel_function(grid, *args, **kwargs):
        ...
        for n in range(core_count - 1):
            ...
            core = PartProcessor(grid[n*grid_section:(n+1)*grid_section,:], target = function, *args, **kwargs)
            core.start()
            cores.append(core)
        core = PartProcessor(grid[(core_count - 1)*grid_section:-1,:], target = function, *args, **kwargs)
        core.start()
        cores.append(core)
        for core in cores:
...
def FisherCounter(k):
    try:
        ...
        def counter(grid, direction_cosines):
            try:
                return np.exp(k*(np.dot(grid, direction_cosines.T) - 1)).sum(axis=1)
            except MemoryError:
                result = np.zeros((grid.shape[0],1))
                for input_node, output_node in zip(grid, result):
                    output_node[:] = np.exp(k*(np.dot(input_node, direction_cosines.T) - 1)).sum()
    return counter
...
def FisherCounterAxial(k):
    try:
        ...
        def counter(grid, direction_cosines):
            try:
                return np.exp(k*(np.abs(np.dot(grid, direction_cosines.T)) - 1)).sum(axis=1)
            except MemoryError:
                result = np.zeros((grid.shape[0],1))
                for input_node, output_node in zip(grid, result):
                    output_node[:] = np.exp(k*(np.abs(np.dot(input_node, direction_cosines.T)) - 1)).sum()
    return counter
...
def RobinGirdleCounter(k):
    try:
        ...
        def counter(grid, direction_cosines):
            try:
                return np.exp(k*(np.dot(grid, direction_cosines.T)**2)).sum(axis=1)
            except MemoryError:
                result = np.zeros((grid.shape[0],1))
                for input_node, output_node in zip(grid, result):
                    output_node[:] = np.exp(k*(np.dot(input_node, direction_cosines.T)**2)).sum()
    return counter
...
k if not given and not available from the DirectionalData options."""
        ...
        try: #It is bettter to beg forgiveness that ask for permittion.
            self.result = np.exp(k*(np.abs(np.dot(self.grid, direction_cosines.T)) - 1)).sum(axis=1)
...
counting angle theta if not given and not available from the DirectionalData options."""
        if isinstance(data, DirectionalData):
            theta = theta if theta is not None else data.kwargs.get('counting_theta', None)
            n = data.n
            direction_cosines = data.data
        else:
            direction_cosines =  data
        ...
        try:
            self.result = np.where(np.abs(np.dot(self.grid, direction_cosines.T)) >= theta, 1, 0).sum(axis=1)
            return self.result
        except MemoryError:
            result = np.zeros((self.grid.shape[0],1))
            for input_node, output_node in zip(self.grid, result):
                output_node[:] = np.where(np.abs(np.dot(input_node, direction_cosines.T)) >= theta, 1, 0).sum()
        self.result = result
        return result
...
class CircularGrid(object):
    ...
    def cdis(self, data, nodes=None, axial=False):
        ...
        d = np.clip(np.dot(nodes, np.transpose(data))/np.linalg.norm(data, axis=1), -1, 1)
        if axial:
            d = np.abs(d)
        return d
    def count(self, data, aperture=None,\
                    axial=False, spacing=None, offset=0, nodes=None, data_weight=None):
        aperture = radians(aperture)/2. if aperture is not None else radians(self.spacing)/2.
        if nodes is None:
            nodes = self.grid if spacing is None else self.build_grid(spacing, offset)
        spacing = radians(self.spacing)/2 if spacing is None else radians(spacing)/2
        c = cos(aperture)
        n = data.shape[0]
    ...
    def count_munro(self, data, weight=.9, aperture=10.,\
        ...
        data_weight = np.ones(data.shape[0]) if data_weight is None else data_weight
        upscale = 1. + 2.*np.power(weight,np.linspace(0., aperture, radians(spacing))).sum()
        return (np.where(d >= c, data_weight, 0)*np.power(weight, theta)).sum(axis=1)[:,None]*upscale/data_weight.sum()


def small_circle_axis(data, concentration_axis=None):
    if concentration_axis is not None:
        data = data*np.where(data.dot(concentration_axis) > 0, 1, -1)[:,None]
